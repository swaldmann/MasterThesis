...
Finally, we can define $x_{i, j} \in \mathbb{N}_i$ as
\begin{equation}
    x_{i, j} = \sigma_{i,j}
\end{equation}
...
The set of neighbors does not make use of the symmetry of selectivities and thus stores all neighbors for each relation explicitly. Thus, we append the parent node $\rho_i$ to the set of neighbors $\mathbb{N}$ calculated for the selectivity generation.
