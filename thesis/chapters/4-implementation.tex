...
For the visualization we create a \texttt{visualization} package to handle all operations related to the visualization itself. Here, we keep track of an exported \texttt{VisualizationOn} Boolean variable to indicate whether the visualization code within the algorithm's implementation should be executed or not. Furthermore, we define two package-local variables:
\vspace{0.2cm}
\begin{minted}{go}
...
\texttt{EnumerateCsg} will calculate all connected subgraphs in the query graph. Thus, the slice of relations \texttt{R} of the query graph \texttt{QG} is traversed in descending order. As discussed in \ref{subsub:basics-bitvector} the relations are stored as a bitvector and individually accessed by shifting $1$ by the repective relation's index to the left.

We make use of arithmetic integer overflow to for the \texttt{for} loop's exit condition as the \texttt{uint} cannot be smaller than 0. In order to get all relations $\mathbb{B}_i$ we shift 1 by $(i+1) - 1$ to the left, leaving us with a 64-bit unsigned integer with 1's for all bits at index $< i$ (Big endian) and $64 - (i-1)$ leading zeroes, representing relations $\{R_{j} \in R \vert j < i\}$.

We keep track of a subgraphs slice which collects the recursively constructed subgraphs from \texttt{EnumerateCsg} and return the result of this slice.
