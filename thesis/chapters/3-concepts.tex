...
\subsubsection{DPccp}
\textbf{Note to Prof. Moerkotte:} Section 3.1 is rather incomplete for now but is only describing the \texttt{DPccp} algorithm, so you can skip to section \ref{sub:concepts-server}.
...
%is possible at least to some degree (TODO: be more precise, e.g. BFS for cycle detection), but not computationally efficient and 
